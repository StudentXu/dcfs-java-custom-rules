...
public class MyJavaRulesDefinitionTest {
  ...
  public void test() {
    ...
    assertThat(repository.name()).isEqualTo("DCITS自定义Java规则");
  ...
  private void assertParameterProperties(Repository repository) {
    ...
    Param max = repository.rule("DontUseTotalData").param("name");
  ...
  private void assertRuleProperties(Repository repository) {
    Rule rule = repository.rule("DontUseTotalData");
    assertThat(rule).isNotNull();
    assertThat(rule.name()).isEqualTo("teller9-用全量数据更新交易数据");
