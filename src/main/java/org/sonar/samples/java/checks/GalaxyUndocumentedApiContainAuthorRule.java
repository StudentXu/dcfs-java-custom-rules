...
@Rule(key = "GalaxyUndocumentedApiContainAuthor")
public class GalaxyUndocumentedApiContainAuthorRule extends BaseTreeVisitor implements JavaFileScanner {
 ...
 /** private static final List<String> methodlist = Arrays.asList("init", "destroy","start","stop");**/
  ...
  private void visitNode(Tree tree, Tree reportTree, SymbolMetadata symbolMetadata) {
    ...
    if (!isExcluded(tree, symbolMetadata)) {
      ...
      if (javadoc == null || isEmptyJavadoc(javadoc)) {
    	...
    	/**if (!javadoc.contains("@author")) {
           context.reportIssue(this, reportTree, "Document this public " + getType(tree) + "with @author.");
     	}**/
  ...
  private static boolean isOverridingMethod(Tree tree) {
	  if (tree.is(Tree.Kind.METHOD)){
		   for (AnnotationTree annotationTree : ((MethodTree) tree).modifiers().annotations()) {
			      if (isJavaLangOverride(annotationTree.annotationType())) {
			        return true;
			      }
			    }
		   /** 
		   if (methodlist.contains(((MethodTree) tree).simpleName().name().toLowerCase())){
			   return true;
		   }
		  **/
		   if (tree.parent().is(CLASS_KINDS) && ((ClassTree) tree.parent()).symbol().isAbstract() && ((MethodTree) tree).symbol().isAbstract()) {
	    		return false;	 
	    	}		
		    		    
		   return !(tree.parent().is(CLASS_KINDS) && ((ClassTree) tree.parent()).symbol().isInterface());
		    
		  }	
	  return !tree.is(Tree.Kind.INTERFACE);
	  /**
 ...
 private static boolean isJavaLangOverride(MemberSelectExpressionTree annotationType) {
  ...
  private WildcardPattern[] getInclusionPatterns() {
    if (inclusionPatterns == null) {
      ...
      inclusionPatterns = DcitsPatternUtils.createPatterns(forClasses);
    }
  ...
  private WildcardPattern[] getExclusionPatterns() {
    if (exclusionPatterns == null) {
      if (StringUtils.isEmpty(exclusion)) {
        ...
        exclusionPatterns = DcitsPatternUtils.createPatterns(exclusion);
      }
    }
